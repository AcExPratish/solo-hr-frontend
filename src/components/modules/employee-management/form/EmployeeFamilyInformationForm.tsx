...
import { EmployeeFamilyInformationSchema } from '@/validation/employee-management/EmployeeSchema';
import Button from '@/components/base/Button';
import { TModalProps } from '@/types/modules';

interface EmployeeFamilyInformationFormProps {
  modal: TModalProps;
  onClose: () => void;
...
const EmployeeFamilyInformationForm = ({
  modal,
  onClose,
  onSubmit,
  formData,
  loading
}: EmployeeFamilyInformationFormProps) => {
  // React Hooks
  ...
  const handleOnSubmit = async (values: TEmployee) => {
    const { _id, user_id, family_information } = values;
    onSubmit({
      _id,
      user_id,
      family_information,
        ...
        {({
          ...
          <Form noValidate onSubmit={handleSubmit}>
            <Modal.Body className="ps-4 pe-4 pt-0">
              ...
              <FieldArray name="family_information">
                {({ push, remove }) => (
                  <div>
                    <Stack gap={4}>
                      {values?.family_information?.map(
                        (data, index: number) => {
                          // Formik Paths
                          const namePath = `family_information.${index}.name`;
                          const relationshipPath = `family_information.${index}.relationship`;
                          const phone1Path = `family_information.${index}.phone_1`;
                          const phone2Path = `family_information.${index}.phone_2`;

                          return (
                            <div key={index}>
                              <h5 className="d-flex justify-content-start mb-3">
                                {`${index + 1}. ${t('family_member_details')}`}
                              </h5>
                              ...
                              <div className="d-flex justify-content-end mt-3">
                                <Button
                                  ...
                                  disabled={
                                    values?.family_information?.length === 1
                                  }
                                >
                      ...
                      <div className="d-flex justify-content-start my-2">
                        ...
                        >
                          ...
                          <span className="ms-1">{t('add_family_member')}</span>
                        </Button>
                      </div>
                    </Stack>
                  </div>
...
export default EmployeeFamilyInformationForm;
